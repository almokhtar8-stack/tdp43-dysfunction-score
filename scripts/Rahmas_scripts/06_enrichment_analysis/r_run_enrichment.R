...
# Use try() to prevent the script from crashing if the internet is slow
kegg_up <- try(enrichKEGG(gene = up_entrez,
                          organism = 'hsa',
                          pvalueCutoff = 0.05,
                          qvalueCutoff = 0.2), 
               silent = TRUE)

# Only process if KEGG succeeded and isn't an error
if (!inherits(kegg_up, "try-error") && !is.null(kegg_up)) {
    ...
    cat("  - KEGG server unavailable or no pathways found. Skipping KEGG.\n\n")
    # Create an empty object so Step 5 doesn't crash
    kegg_up <- list(result = data.frame()) 
}
...
if (!is.list(kegg_up) && nrow(kegg_up@result[kegg_up@result$p.adjust < 0.05,]) > 0) {
  p3 <- barplot(kegg_up, 
                showCategory = 15,
...
if (!is.list(kegg_up) && nrow(kegg_up@result) > 0) {
...
cat("Results Summary:\n")
cat("  GO Biological Process (up):", nrow(go_bp_up@result[go_bp_up@result$p.adjust < 0.05,]), "terms\n")
cat("  GO Molecular Function (up):", nrow(go_mf_up@result[go_mf_up@result$p.adjust < 0.05,]), "terms\n")
cat("  GO Cellular Component (up):", nrow(go_cc_up@result[go_cc_up@result$p.adjust < 0.05,]), "terms\n")

# Safe check for KEGG
if (is.list(kegg_up)) {
  cat("  KEGG Pathways (up): 0 pathways (Server unavailable)\n")
} else {
  cat("  KEGG Pathways (up):", nrow(kegg_up@result[kegg_up@result$p.adjust < 0.05,]), "pathways\n")
}

cat("  GO Biological Process (down):", nrow(go_bp_down@result[go_bp_down@result$p.adjust < 0.05,]), "terms\n\n")
