...
# Load the main significance table
sig_results <- read.csv(file.path(TABLES_DIR, "deseq2_results_significant.csv"))

# Filter Up and Down right here in the script
up_genes <- sig_results %>% filter(log2FoldChange > 1)
down_genes <- sig_results %>% filter(log2FoldChange < -1)

cat("  - Upregulated detected:", nrow(up_genes), "\n")
cat("  - Downregulated detected:", nrow(down_genes), "\n\n")
